...
class Config:
            ...
            with open(filename, 'r', encoding="utf-8") as f:
                for line in f:
                    ...
                    if key in {"start_hub", "end_hub"}:
                        ...
                        if hub.max_drones < all_keys["nb_drones"]["val"]:
                            err_msg = "start or end has insufficient capacity"
                            raise ValueError(err_msg)
                        # add to hub_list
                        all_keys["hub"]["val"].append(hub)
                        # add as start and end
                        ...
                        parser_func = all_keys[key]["func"]
                        new_c = parser_func(value_str)
                        for c in all_keys[key]["val"]:
                            if (c.hub1 == new_c.hub1
                                    and c.hub2 == new_c.hub2):
                                raise ValueError("duplicate connection found")
                        all_keys[key]["val"].append(new_c)
                    else:
    ...
    def _parse_hub(value_str: str, s_or_e: str | None = None) -> Hub:
        ...
        also default for the start and end drones is inf, otherwise
        it uses max_drones metadata value
        """
        ...
        if len(params) == 3:
            return Hub(
                ...
                max_drones=1 if s_or_e is None else inf
                )
        ...
        if s_or_e:
            max_drones = metadata.get(Metadata.MAX_DRONES, inf)
        else:
